...
9. SQLite persistence upserts normalized jobs, deduped jobs, classification
   results, ranking scores, and match history, while recording each pipeline run.
10. Pipeline writes a local JSON report.
11. GCS helpers summarize program-level trend fields separately from ranking.
12. Evaluation reads labelled fixtures and reports classifier/ranking metrics.
